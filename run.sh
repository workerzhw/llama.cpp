...
validate_case() {
  ...
  if ! [[ "${FP8_SIM_STATS_SAMPLE}" =~ ^[0-9]+$ ]] || [[ "${FP8_SIM_STATS_SAMPLE}" -le 0 ]]; then
    echo "invalid FP8_SIM_STATS_SAMPLE=${FP8_SIM_STATS_SAMPLE} (expected positive integer)" >&2
    exit 1
  fi
