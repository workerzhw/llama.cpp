...
void ggml_zlpu_flash_attn_ext(ggml_backend_zlpu_context & ctx, ggml_tensor * dst)
{
    GGML_UNUSED(ctx);
    GGML_UNUSED(dst);
}
void ggml_zlpu_softmax(ggml_backend_zlpu_context & ctx, ggml_tensor * dst) {
    ggml_tensor * src0 = dst->src[0];
    ggml_tensor * src1 = dst->src[1];  // mask

    // softmax
    // aclnn_softmax(ctx, softmax_tensor, 3, acl_dst);
    // ggml_zlpu_release_resources(ctx, acl_src0, acl_dst, acl_scale, softmax_tensor);
    GGML_UNUSED(ctx);
    GGML_UNUSED(dst);
}
...
void ggml_zdnn_mul_mat_f(
    const ggml_backend_zlpu_context & ctx,
